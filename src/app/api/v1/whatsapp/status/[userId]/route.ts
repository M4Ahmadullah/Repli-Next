  ...
  try {
    ...
    if (!userId) {
      ...
      if (authHeader && authHeader.startsWith('Bearer ')) {
        // For now, we'll use the paramUserId when we have a Bearer token
        // This indicates a direct API call with Firebase token
    ...
    const response = await fetch(`${BOT_SYSTEM_URL}/api/v1/whatsapp/status/${paramUserId}`, {
      method: 'GET',
      headers: {
