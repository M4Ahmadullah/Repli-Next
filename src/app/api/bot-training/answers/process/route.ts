...
export async function POST(request: NextRequest) {
  try {
    ...
    const botResponse = await fetch(`${BOT_API_BASE_URL}/api/v1/bot-training/answers/process`, {
      method: 'POST',
      headers: {
    ...
    if (!botResponse.ok) {
      ...
      return NextResponse.json({
        success: false,
        error: errorData.error || 'Failed to process answers',
    ...
    console.error('Answer processing proxy error:', error)
    return NextResponse.json({ 
      success: false,
