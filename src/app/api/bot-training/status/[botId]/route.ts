...
export async function GET(
  ...
  try {
    ...
    const botResponse = await fetch(`${BOT_API_BASE_URL}/api/v1/bot-training/status/${botId}`, {
      method: 'GET',
      headers: {
    ...
    if (!botResponse.ok) {
      ...
      return NextResponse.json({
        success: false,
        error: errorData.error || 'Failed to get training status',
    ...
    console.error('Training status proxy error:', error)
    return NextResponse.json({ 
      success: false,
